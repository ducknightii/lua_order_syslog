...
function M.qhinit(category, facility)
  if not facility then facility = FACILITY.USER
  else facility = FACILITY[facility:upper()] end
  ...
  return function (msg, lvl, now)
    ...
    -- body
    local Y, M, D = date():getdate()
    local h, m, s = date():gettime()
    local time =fmt("%.4d-%.2d-%.2d %.2d:%.2d:%.2d", Y, M, D, h, m, s)
    local level_name = Log.LVL_NAMES[lvl]

    local message = msg["message"] or ""
    local context = ""
    if msg["context"] then
      context =  cjson.encode(msg["context"])
      message = category .. '.' .. message
    end

    local body = message .. ' ' .. level_name .. "\t" .. '[' .. time .. ']' .. "\t" .. context
    ...
    local extra = cjson.encode(_extra)
